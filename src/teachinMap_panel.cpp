    ...
    : rviz_common::Panel(parent), node_(rclcpp::Node::make_shared("teachinMap_panel_node"))
{
    ...
    odom_sub_ = node_->create_subscription<nav_msgs::msg::Odometry>(
        "/ia01/odom", 10, std::bind(&TeachinMapPanel::odomCallback, this, std::placeholders::_1));
    context_ = nullptr;
}
...
void TeachinMapPanel::onAddCurrentPositionButtonClicked()
{
    ...
    RCLCPP_INFO(node_->get_logger(), "Add position: %.2f, %.2f, %.2f", p.x, p.y, p.z);
...
void TeachinMapPanel::onClearButtonClicked()
{
    polygon_points_.clear();
    RCLCPP_INFO(node_->get_logger(), "Polygon points cleared.");
    auto request = std::make_shared<stihl_nav_msgs::srv::PostPolygonToMap::Request>();
    request->action = "update_polygon";
    auto result = polygon_client_->async_send_request(request);
    if (rclcpp::spin_until_future_complete(node_, result) == rclcpp::FutureReturnCode::SUCCESS) {
        RCLCPP_INFO(node_->get_logger(), "Polygon cleared successfully on the server.");
    } else {
        RCLCPP_ERROR(node_->get_logger(), "Failed to clear polygon on the server.");
    }
}
